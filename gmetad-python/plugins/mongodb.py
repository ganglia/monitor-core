...
class MongodbPlugin(GmetadPlugin) :
    ...
    def notify(self, clusterNode):
        ...
        for hostNode in clusterNode:
            ...
            vm, obj = find_object(hostNode.getAttr('ip'), self)

            if vm is None : # error during lookup
                continue
            
            data = {"uuid" : obj["uuid"], "expid" : self.expid}
            ...
            for metricNode in hostNode:
                ...
                data[attrs["name"]] = attrs 

                empty = False
                        
            if not empty :
                _now = int(time())
                data["time_h"] = makeTimestamp(_now)
                data["time"] = _now 
                data["latest_update"] = _now
                
                try :
                    self.msci.add_document(self.data_collection[obj_type], data)
                    data["_id"] = obj["uuid"]
                    self.msci.update_document(self.latest_collection[obj_type], data)

                except Exception, e :
                    _status = 23
                    _fmsg = str(e)
                    logging.error("Could not write to metric store: " + str(msg))
