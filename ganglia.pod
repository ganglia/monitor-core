...
package repositories, all of them unofficial, it is pretty difficult to be sure
that all possible permutations had been confirmed to work reliably.

Be sure to have all dependencies as explained in the INSTALL file covered and
to use GNU make and a gcc compiler that builds 32bit binaries with all other
libraries matching that ISA.

When in doubt, build that problematic dependency from source and remember
to distribute it together with your ganglia build as everything is dynamically
linked by default.

Be specially careful about libConfuse (specially if using the old 2.5 version)
which has been known to be incorrectly packaged and to compile by default
only as a static library that will then fail to link with ganglia.
...
the buid system and the way gmond uses to locate its metrics had to be
changed in a very drastic way and therefore getting gmond to build and work
again required fixes to be implemented for all platforms.
...
yet, but if you have access to any of them and want to run ganglia 3.1
...
going to be available in Debian experimental very soon.
