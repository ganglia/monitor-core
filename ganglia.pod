...
Solaris, FreeBSD, AIX, IRIX, Tru64, HPUX, MacOS X and Windows (cygwin beta) making it as 
...
place the C<ganglia/> directory anywhere convenient. I like to unzip *tar.gz 
...
is current as of ganglia 2.5.6).
  ...
  cpu_aidle     Percent of time since boot idle CPU      l,h
  ...
  mem_arm       Available real memory                    h
  mem_avm       Available virtual memory                 h
  mem_buffers   Amount of buffered memory                l,f,i,t
  mem_cached    Amount of cached memory                  l,f,i,h
  mem_free      Amount of available memory               e
  mem_rm        Total real memory                        h
  mem_shared    Amount of shared memory                  l,f,i,h
  mem_total     Amount of available memory               e 
  mem_vm        Total virtual memory                     h
  mtu           Network maximum transmission unit        e 
  os_name       Operating system name                    e 
  os_release    Operating system release (version)       e 
  ...
  pkts_in       Packets in per second                    l,f,s
  pkts_out      Packets out per second                   l,f,s
  proc_run      Total number of running processes        l,f,s,m,i,h
  proc_total    Total number of processes                l,f,s,m,i,h
  rcache        Read cache hit ratio                     s
  swap_free     Amount of available swap memory          l,f,s,a,i,h,t
  swap_total    Total amount of swap memory              l,f,s,a,i,h,t
  sys_clock     Current time on host                     e
...
take a look in directory C<./gmond/machines> in the source distribution.
...
well-formed XML.  C<xmllint> will print the entire XML stream if there are no errors.  If 
...
=item 1.
All hosts in a cluster are multicasting on the ganglia channel.
...
=head1 Copyright
  ...
  Copyright (C) 2002,2003,2004 University of California, Berkeley

=head1 Authors

The B<Ganglia Development Team>...

 Matt Massie            massie          Project Lead    massie at users.sourceforge.net
