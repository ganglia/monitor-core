...
B<debug_level>, B<mute>, B<deaf>, B<host_dmax>, B<cleanup_threshold>,
B<gexec> and B<send_metadata_interval>.
...
B<mcast_if>, B<host>, B<port> and B<ttl>.
...
The B<timeout> attribute allows you to specify how many microseconds to block before
closing a connection to a client.  The default is set to 1 second (1000000 usecs).  
...
The B<metric> section has the following attributes: B<name>,
B<value_threshold> and B<title>.  For a list of available metric 
names, run the following command:
...
are the directives B<name>, B<path> and B<params>.  The module B<name> is the name
of the module as determined by the module structure. The B<path> is the path
from which gmond is expected load the module.  The B<params> directive can 
be used to pass a single string parameter directly to the module initialization 
function. Multiple parameters can be passed to the module's initialization 
function by including one or more B<param> sections. Each B<param> section 
must be named and contain a B<value> directive. Once a module as been loaded,
the additional metrics can be discovered by invoking B<gmond -m>.
