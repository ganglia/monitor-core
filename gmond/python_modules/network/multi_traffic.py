...
def metric_init(params):
    ...
    print '[multi_traffic] Received the following parameters'
    print params
    ...
    # generate the target device param list , each item is like single traffic1 did before
    params_list = []
    def filter_no_device(x):
        if not x[0].startswith('target_device'):
            return True
        else:
            return False
        
    no_device_params = dict(filter(filter_no_device, params.items()))

    for k,v in params.items():
        if k.startswith('target_device'):
            p = copy.deepcopy(no_device_params)
            p['target_device'] = v
            params_list.append(p)
    
    for params_one in params_list:

        _Worker_Thread = UpdateTrafficThread(params_one)
        _Worker_Thread.start()
    
        # IP:HOSTNAME
        if "spoof_host" in params_one:
            Desc_Skel["spoof_host"] = params_one["spoof_host"]
    
        descriptors.append(create_desc(Desc_Skel, {
                    "name"        : "recv_bytes_%s" % params_one['target_device'],
                    "units"       : "bytes/sec",
                    "description" : "received bytes per sec",
                    }))
        descriptors.append(create_desc(Desc_Skel, {
                    "name"        : "recv_pkts_%s" % params_one['target_device'],
                    "units"       : "pkts/sec",
                    "description" : "received packets per sec",
                    }))
        descriptors.append(create_desc(Desc_Skel, {
                    "name"        : "recv_errs_%s" % params_one['target_device'],
                    "units"       : "pkts/sec",
                    "description" : "received error packets per sec",
                    }))
    
        descriptors.append(create_desc(Desc_Skel, {
                    "name"        : "trans_bytes_%s" % params_one['target_device'],
                    "units"       : "bytes/sec",
                    "description" : "transmitted bytes per sec",
                    }))
        descriptors.append(create_desc(Desc_Skel, {
                    "name"        : "trans_pkts_%s" % params_one['target_device'],
                    "units"       : "pkts/sec",
                    "description" : "transmitted packets per sec",
                    }))
        descriptors.append(create_desc(Desc_Skel, {
                    "name"        : "trans_errs_%s" % params_one['target_device'],
                    "units"       : "pkts/sec",
