...
hash_create (size_t size)
{
   ...
   hash->node = calloc(hash->size, sizeof (node_t));
   if (hash->node == NULL)
      {
...
hash_insert (datum_t *key, datum_t *val, hash_t *hash)
{
  ...
  ck_rwlock_write_lock(&hash->lock);
        ...
        if ( bucket->key == NULL )
           {
              free(bucket);
              bucket = NULL;
              ck_rwlock_write_unlock(&hash->lock);
              return NULL;
           } 
...
hash_lookup (datum_t *key, hash_t * hash)
{
  ...
  ck_rwlock_read_lock(&hash->lock);

  bucket = &hash->node[i];

  if ( bucket == NULL )
     {
        ck_rwlock_read_unlock(&hash->lock);
        return NULL;
     }
